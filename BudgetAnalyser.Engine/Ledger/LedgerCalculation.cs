...
public class LedgerCalculation
{
    ...
    /// </summary>
    public decimal CalculateCurrentPeriodBucketSpend(LedgerEntryLine ledgerLine, GlobalFilterCriteria filter, TransactionsListModel transactionsList, string bucketCode)
    {
        ...
        {
            throw new ArgumentNullException(nameof(transactionsList));
        }
        ...
        var transactionTotal = CalculateTransactionTotal(filter.BeginDate.Value, filter.EndDate.Value, transactionsList, ledgerLine, bucketCode);
    ...
    public virtual IDictionary<BudgetBucket, decimal> CalculateCurrentPeriodLedgerBalances(LedgerEntryLine ledgerLine, GlobalFilterCriteria filter, TransactionsListModel transactionsList)
    {
        CheckCacheForCleanUp();
        if (ledgerLine is null)
        {
        ...
        ledgers = CalculateLedgersCurrentBalances(ledgerLine, filter.BeginDate.Value, filter.EndDate.Value, transactionsList);

        // Check Surplus
        var surplusBalance = CalculateCurrentPeriodSurplusBalance(ledgerLine, filter, transactionsList);
    ...
    /// </summary>
    public decimal CalculateCurrentPeriodSurplusBalance(LedgerEntryLine ledgerLine, GlobalFilterCriteria filter, TransactionsListModel transactionsList)
    {
        CheckCacheForCleanUp();
        if (ledgerLine is null)
        {
        ...
        var transactionTotal = CalculateTransactionTotal(filter.BeginDate.Value, filter.EndDate.Value, transactionsList, ledgerLine, SurplusBucket.SurplusCode);
        ...
        var ledgersSummary = CalculateLedgersCurrentBalances(ledgerLine, filter.BeginDate.Value, filter.EndDate.Value, transactionsList);
    ...
    /// </summary>
    public IEnumerable<ReportTransaction> CalculateOverSpentLedgers(TransactionsListModel transactionsList, LedgerEntryLine ledgerLine, DateOnly inclBeginDate, DateOnly inclEndDate)
    {
        CheckCacheForCleanUp();
        ...
        List<ReportTransaction> GetOverSpentTransactions() //private function
        {
            ...
            do
            {
                ...
                foreach (var transaction in transactionsList.Transactions.Where(t => t.Date == currentDateCopy && t.BudgetBucket is not null))
                {
        ...
        var cacheKey = BuildCacheKey("CalculateOverSpentLedgers", transactionsList, ledgerLine, inclBeginDate);
    ...
    private Dictionary<BudgetBucket, decimal> CalculateLedgersCurrentBalances(LedgerEntryLine ledgerLine, DateOnly inclBeginDate, DateOnly inclEndDate, TransactionsListModel transactionsList)
    {
        var ledgersSummary = new Dictionary<BudgetBucket, decimal>();
        foreach (var entry in ledgerLine.Entries)
        {
            var transactionTotal = CalculateTransactionTotal(inclBeginDate, inclEndDate, transactionsList, ledgerLine, entry.LedgerBucket.BudgetBucket.Code);
    ...
    private decimal CalculateTransactionTotal(DateOnly inclBeginDate, DateOnly inclEndDate, TransactionsListModel transactionsList, LedgerEntryLine entryLine, string bucketCode)
    {
        var autoMatchLedgerTransactions = GetAutoMatchingTransactions(transactionsList, entryLine, inclBeginDate);
        ...
        var transactions = transactionsList.AllTransactions
        ...
        this.logger.LogInfo(_ =>
        {
            ...
            builder.AppendLine($"Transactions found that are '{bucketCode}' and not 'Auto-Matching-Transactions':");
    ...
    private IEnumerable<LedgerTransaction> GetAutoMatchingTransactions(TransactionsListModel transactionsList, LedgerEntryLine entryLine, DateOnly inclBeginDate)
    {
