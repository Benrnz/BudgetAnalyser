...
[AutoRegisterWithIoC]
internal class ReconciliationCreationManager(
    ITransactionRuleService transactionRuleService,
    IReconciliationConsistency reconciliationConsistency,
    IReconciliationBuilder builder,
    ILogger logger)
    : IReconciliationCreationManager
{
    private readonly IReconciliationBuilder builder = builder ?? throw new ArgumentNullException(nameof(builder));
    private readonly ILogger logger = logger ?? throw new ArgumentNullException(nameof(logger));
    private readonly IReconciliationConsistency reconciliationConsistency = reconciliationConsistency ?? throw new ArgumentNullException(nameof(reconciliationConsistency));
    private readonly ITransactionRuleService transactionRuleService = transactionRuleService ?? throw new ArgumentNullException(nameof(transactionRuleService));
        ...
        params BankBalance[] currentBankBalances)
    {
        ...
        var budgetToUse = budgetCollection.ForDate(closingDateExclusive)
                          ?? throw new ArgumentNullException($"No budget found to use for the reconciliation date {closingDateExclusive}");

        try
        {
            PreReconciliationValidation(ledgerBook, closingDateExclusive, transactionSet, budgetToUse);
        }
        ...
        try
        {
            using (this.reconciliationConsistency.EnsureConsistency(ledgerBook))
            {
                ...
                recon = this.builder.CreateNewMonthlyReconciliation(closingDateExclusive, budgetToUse, transactionSet, currentBankBalances);
        ...
        foreach (var task in recon.Tasks)
        {
            ...
            {
                this.logger.LogInfo(l => l.Format(
                ...
                this.transactionRuleService.CreateNewSingleUseRule(transferTask.BucketCode, null, [transferTask.Reference], null, null, true);
            }
        }
    ...
    private void PerformBankTransfer(LedgerBook ledgerBook, TransferFundsCommand transferDetails, LedgerEntryLine ledgerEntryLine)
    {
        ...
        if (transferDetails.BankTransferRequired)
        {
            ...
            this.transactionRuleService.CreateNewSingleUseRule(
                ...
                null,
                [transferDetails.AutoMatchingReference],
                null,
    ...
    private void PreReconciliationValidation(LedgerBook ledgerBook, DateOnly reconciliationDate, TransactionSetModel transactionSet, BudgetModel budget)
    {
        ...
        ValidateDates(ledgerBook, startDate, reconciliationDate, transactionSet, budget.BudgetCycle);

        ValidateAgainstUncategorisedTransactions(startDate, reconciliationDate, transactionSet);

        ValidateAgainstOrphanedAutoMatchingTransactions(ledgerBook, transactionSet);

        ValidateAgainstMissingTransactions(reconciliationDate, transactionSet);
    }
    ...
    private bool ShouldValidationExceptionBeRethrown(bool ignoreWarnings, ValidationWarningException ex)
    {
        if (ignoreWarnings)
        {
            if (ex.Source is not null && this.validationMessages.Contains(ex.Source))
            {
                return false;
            }

            // It's a new warning not previously seen.
            return true;
        }
    ...
    private static void ValidateAgainstMissingTransactions(DateOnly reconciliationDate, TransactionSetModel transactionSet)
    {
        var lastTransactionDate = transactionSet.Transactions.Where(t => t.Date < reconciliationDate).Max(t => t.Date);
    ...
    private void ValidateAgainstUncategorisedTransactions(DateOnly startDate, DateOnly reconciliationDate, TransactionSetModel transactionSet)
    {
        if (transactionSet.AllTransactions
        ...
        {
            var uncategorised =
                transactionSet.AllTransactions.Where(
                    t =>
                        t.BudgetBucket is null ||
                        (t.BudgetBucket is not null && string.IsNullOrWhiteSpace(t.BudgetBucket.Code)));
            var count = 0;
    ...
    private static void ValidateDates(LedgerBook ledgerBook, DateOnly startDate, DateOnly reconciliationDate, TransactionSetModel transactionSet, BudgetCycle periodType)
    {
        ...
        if (previousEntry is not null)
        {
            ...
            switch (periodType)
            {
                case BudgetCycle.Monthly:
                    ...
                    {
                        throw new ValidationWarningException(
                            $"The reconciliation date chosen, {reconciliationDate}, isn't the same day of the month as the previous entry {previousEntry.Date}. Not required, but ideally reconciliations should be evenly spaced.")
                        {
                            Source = "4"
                        };
                    }
        ...
        if (!transactionSet.AllTransactions.Any(t => t.Date >= startDate))
        {
