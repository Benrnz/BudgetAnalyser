...
namespace BudgetAnalyser.Engine.Persistence
{
    ...
    public class XamlOnDiskApplicationDatabaseRepository : IApplicationDatabaseRepository
    {
        ...
        /// </exception>
        public XamlOnDiskApplicationDatabaseRepository([NotNull] IDtoMapper<BudgetAnalyserStorageRoot, ApplicationDatabase> mapper)
        {
            if (mapper == null)
            {
        ...
        /// <exception cref="DataFormatException">Deserialisation Application Database file failed, an exception was thrown by the Xml deserialiser, the file format is invalid.</exception>
        ...
        public async Task SaveAsync(ApplicationDatabase budgetAnalyserDatabase)
        {
            ...
            string serialised = Serialise(this.mapper.ToDto(budgetAnalyserDatabase));
            await WriteToDiskAsync(budgetAnalyserDatabase.FileName, serialised);
        }

        /// <summary>
        /// Checks to see if the budget analyser identified by the <paramref name="budgetAnalyserDataStorage"/> exists in storage or not.
        /// </summary>
