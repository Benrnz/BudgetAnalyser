...
/// <summary>
///     A collection of bank transactions that have been imported.
/// </summary>
...
public class TransactionsListModel : INotifyPropertyChanged, IDataChangeDetection, IDisposable
{
    private readonly ILogger logger;

    /// <summary>
    ///     A hash to show when critical state of the transactions list model has changed. Includes child objects ie Transactions. The hash does not persist between Application Loads.
    /// </summary>
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="TransactionsListModel" /> class.
    /// </summary>
    ...
    public TransactionsListModel(ILogger logger)
    {
    ...
    /// <summary>
    ///     Gets a value indicating whether this <see cref="TransactionsListModel" /> has an active filtered.
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets the storage key.  This could be the filename for the transactions list model's persistence, or a database unique id.
    /// </summary>
    ...
    /// <summary>
    ///     Used internally by the importers to load transactions into the transactions list model. This includes data loaded by persistence.
    /// </summary>
    /// <param name="transactions">The transactions to load.</param>
    /// <returns>Returns this instance, to allow chaining.</returns>
    internal TransactionsListModel LoadTransactions(IEnumerable<Transaction> transactions)
    {
        ThrowIfDisposed();
        UnsubscribeToTransactionChangedEvents();
        this.changeHash = Guid.NewGuid();
    ...
    internal virtual TransactionsListModel Merge(TransactionsListModel additionalModel)
    {
        ThrowIfDisposed();
        ...
        var combinedModel = new TransactionsListModel(this.logger) { LastImport = additionalModel.LastImport, StorageKey = StorageKey };
    ...
    internal IEnumerable<IGrouping<int, Transaction>> ValidateAgainstDuplicates(DateOnly? minDate = null, DateOnly? maxDate = null)
    {
        ...
        if (this.duplicates is not null)
        {
            ...
            // Reset by Merging Transactions, Load Transactions, or by reloading the transactions list model.
        }
    ...
    private void ThrowIfDisposed()
    {
        if (this.disposed)
        {
            throw new ObjectDisposedException(nameof(TransactionsListModel));
        }
    }
    ...
    /// <summary>
    ///     Finalizes an instance of the <see cref="TransactionsListModel" /> class. This destructor will run only if the Dispose method does not get called.
