...
public class TransactionSetModel : INotifyPropertyChanged, IDataChangeDetection, IDisposable
{
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="TransactionSetModel" /> class.
    /// </summary>
    ...
    public TransactionSetModel(ILogger logger)
    {
    ...
    /// <summary>
    ///     Gets a value indicating whether this <see cref="TransactionSetModel" /> has an active filtered.
    /// </summary>
    ...
    internal TransactionSetModel LoadTransactions(IEnumerable<Transaction> transactions)
    {
        ThrowIfDisposed();
        UnsubscribeToTransactionChangedEvents();
        this.changeHash = Guid.NewGuid();
    ...
    internal virtual TransactionSetModel Merge(TransactionSetModel additionalModel)
    {
        ThrowIfDisposed();
        ...
        var combinedModel = new TransactionSetModel(this.logger) { LastImport = additionalModel.LastImport, StorageKey = StorageKey };
    ...
    private void ThrowIfDisposed()
    {
        if (this.disposed)
        {
            throw new ObjectDisposedException(nameof(TransactionSetModel));
        }
    }
    ...
    /// <summary>
    ///     Finalizes an instance of the <see cref="TransactionSetModel" /> class. This destructor will run only if the Dispose method does not get called.
