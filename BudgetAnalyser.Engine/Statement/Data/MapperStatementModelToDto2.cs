    ...
    ILogger logger) : IDtoMapper<TransactionSetDto, StatementModel>
{
    ...
    public TransactionSetDto ToDto(StatementModel model)
    {
        ...
        var dto = new TransactionSetDto { LastImport = model.LastImport, StorageKey = model.StorageKey, Transactions = model.AllTransactions.Select(this.transactionMapper.ToDto).ToList() };
    ...
    public StatementModel ToModel(TransactionSetDto dto)
    {
        ...
        var model = new StatementModel(this.logger) { StorageKey = dto.StorageKey, LastImport = dto.LastImport };
