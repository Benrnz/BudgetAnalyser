...
namespace BudgetAnalyser.Engine.Account
{
    ...
    public class InMemoryAccountTypeRepository : IAccountTypeRepository
    {
        ...
        /// <summary>
        ///     Retrieve the <see cref="AccountType" /> for the given key or creates a new one if it doesn't exist in the
        ///     repository.
        /// </summary>
        /// <param name="key">The unique key.</param>
        /// <param name="factory">
        ///     The delegate to create a new <see cref="AccountType" />. If not specified, the repository will
        ///     "guess" which type to create.
        /// </param>
        /// <returns>The found account type or a reference to the newly created one.</returns>
        public AccountType GetOrCreateNew(string key, Func<string, AccountType> factory = null)
        {
            if (string.IsNullOrWhiteSpace(key))
            {
                return null;
            ...
            string casedKey = key.ToUpperInvariant();
            return this.repository.GetOrAdd(casedKey, CreateNewAccountType);
        }
