...
/// <summary>
///     A service to provide access to manipulate ledger books. This service is designed to be stateful.
/// </summary>
public interface ILedgerService : INotifyDatabaseChanges, IServiceFoundation
{
    ...
    void MoveLedgerToAccount(LedgerBucket ledger, Account storedInAccount);

    /// <summary>
    ///     Renames the ledger book.
    /// </summary>
    /// <param name="newName">The new name.</param>
    void RenameLedgerBook(string newName);

    /// <summary>
    ///     Tracks a new budget bucket by creating a new <see cref="LedgerBucket" /> for the given <see cref="BudgetBucket" /> and adds the ledger to the ledger book.
    /// </summary>
    /// <param name="bucket">The bucket to track.</param>
    /// <param name="storeInThisAccount">The account to store the ledger's funds.</param>
    void TrackNewBudgetBucket(ExpenseBucket bucket, Account storeInThisAccount);
