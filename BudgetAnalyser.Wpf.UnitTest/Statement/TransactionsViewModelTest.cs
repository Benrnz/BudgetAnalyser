...
public class TransactionsViewModelTest
{
    ...
    {
        var subject = CreateSubject();
        Assert.AreEqual("[No Transactions Loaded]", subject.TransactionsModelName);
    }
    ...
    {
        var subject = Arrange();
        Assert.AreEqual("FooStatement", subject.TransactionsModelName);
    }
    ...
    private TransactionsViewModel Arrange()
    {
        var subject = CreateSubject();
        subject.TransactionsModel = StatementModelTestData.TestData1();
        return subject;
    }

    private TransactionsViewModel CreateSubject()
    {
        return new TransactionsViewModel(new Mock<IApplicationDatabaseFacade>().Object, this.mockTransactionService.Object);
    }
