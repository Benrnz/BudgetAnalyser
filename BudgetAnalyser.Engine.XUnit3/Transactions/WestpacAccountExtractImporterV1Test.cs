...
public class WestpacAccountExtractImporterV1Test
{
    ...
    {
        Should.Throw<ArgumentNullException>(() => new WestpacAccountExtractImporterV1(null!, new FakeLogger(), this.readerWriterSelector));
    }

    [Fact]
    public void CtorShouldThrowGivenNullLogger()
    {
        Should.Throw<ArgumentNullException>(() => new WestpacAccountExtractImporterV1(new BankImportUtilities(new FakeLogger()), null!, this.readerWriterSelector));
    }
    ...
    {
        var subject = Arrange();
        subject.ReadLinesOverride = _ => WestpacChequeCsvTestData.TestData2();
    ...
    {
        var subject = Arrange();
        subject.ReadLinesOverride = _ => WestpacChequeCsvTestData.TestData1();
        var result = await subject.LoadAsync("foo.bar", TransactionsListModelTestData.ChequeAccount);
    ...
    {
        var subject = Arrange();
        subject.ReadLinesOverride = _ => WestpacChequeCsvTestData.BadTestData1();
    ...
    {
        return new WestpacAccountExtractImporterV1TestHarness(this.bankImportUtilities, this.readerWriterSelector);
    }
}
