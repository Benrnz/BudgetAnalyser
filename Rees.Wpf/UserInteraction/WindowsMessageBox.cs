...
public class WindowsMessageBox : MessageBoxBase
{
    ...
    [SuppressMessage("Microsoft.Design", "CA1026:DefaultParametersShouldNotBeUsed",
        Justification = "Optional parameters are preferred unless theres a reason not to.")]
    public override void Show(string message, string headingCaption = "")
    {
        ...
        try
        {
            heading = string.IsNullOrWhiteSpace(headingCaption) ? window?.Title ?? string.Empty : headingCaption;
        }
        ...
        try
        {
            ...
            {
                MessageBox.Show(window, content, heading);
            }
        }
        catch (InvalidOperationException)
        {
    ...
    /// <summary>
    ///     Show a message using a string format.
    /// </summary>
