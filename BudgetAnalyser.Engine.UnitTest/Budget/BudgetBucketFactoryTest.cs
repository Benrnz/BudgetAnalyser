...
{
    private IBudgetBucketFactory Subject { get; set; }
    ...
    public void BuildShouldCreateFixedBudgetProject()
    {
        Assert.IsInstanceOfType(
            Subject.BuildModel(
                new FixedBudgetBucketDto { Type = BucketDtoType.FixedBudgetProject, Created = DateTime.Now, Code = "SURPLUS.FOO", Description = "Fu bar" }),
    ...
    public void BuildShouldCreateIncomeBucket()
    {
        Assert.IsInstanceOfType(
            Subject.BuildModel(
                new BudgetBucketDto { Type = BucketDtoType.Income }),
