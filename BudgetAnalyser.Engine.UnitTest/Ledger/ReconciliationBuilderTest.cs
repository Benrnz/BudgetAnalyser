...
public class ReconciliationBuilderTest
{
    ...
    public void MonthEndReconciliation_ShouldCreateSingleUseMatchingRulesForTransferToDos()
    {
        ...
        ((LedgerBookTestHarness)this.testDataLedgerBook).ReconcileOverride = recon =>
        {
            this.testDataToDoList.Add(
                                      new TransferTask(string.Empty, true)
                                      {
                                          Reference = "sjghsh",
                                          Amount = 12.22M,
        ...
        this.mockReconciliationBuilder.Setup(m => m.CreateNewMonthlyReconciliation(ReconcileDate, this.testDataBudgetContext.Model, this.testDataStatement, It.IsAny<BankBalance[]>()))
            .Returns(new ReconciliationResult { Tasks = this.testDataToDoList });
    ...
    public void Reconcile_ShouldAutoMatchTransactionsAndUpdateLedgerAutoMatchRefSoItIsNotAutoMatchedAgain_GivenTestData5()
    {
        ...
        ActPeriodEndReconciliationOnTestData5();
        var previousMonthLine = this.testDataLedgerBook.Reconciliations.Single(line => line.Date == ReconcileDate.AddMonths(-1))
    ...
    public void Reconcile_ShouldNotThrow_GivenTestData1AndUnclassifiedTransactionsOutsideReconPeriod()
    {
        ...
        this.mockReconciliationBuilder.Setup(m => m.CreateNewMonthlyReconciliation(reconDate, this.testDataBudgetContext.Model, this.testDataStatement, It.IsAny<BankBalance[]>()))
            .Returns(new ReconciliationResult { Tasks = this.testDataToDoList });
